...
breed [firms firm]
patches-own [land-lord-ID]
people-own [firm! land-cost lov budget spending-on-land spending-on-goods utility income]
landlords-own [base-land-cost net-stock-level home-x home-y p-color ID]
...
to setup-landlord-patches
  ...
  let patches-assigned count patches with [land-lord-ID > 0]
  let assigned% patches-assigned / patches-count * 100
  ifelse (assigned% > tipping-point)
  [

    ask patches with [land-lord-ID = 0] [
      let neigbour-ID 0
    let neigbour-pcol black
      ask neighbors4 [
        if(land-lord-ID > 0)
        [
          set neigbour-pcol pcolor
          set neigbour-ID land-lord-ID
          stop
        ]
      ]
       set  pcolor neigbour-pcol
       set  land-lord-ID neigbour-ID
  ...
  ask landlords [
    ...
    ask patch-here[set pll-id  land-lord-ID ]

    ifelse (pll-id = 0)
    [
      set pcolor p-color
      ask patch-here[ set land-lord-ID llID   ]
    ]
    [
      ifelse ( pll-id != ID)
      [] ;do nothing
